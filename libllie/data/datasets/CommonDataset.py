"""Common paired dataset implementation for_teach low-light enhancement."""
...
class CommonDataset(BaseDataset):
    """Common paired low-light enhancement dataset.

    This dataset supports common directory layouts used by low-light image
    enhancement datasets. The preferred paired layout is:
    ...
    It also supports compatible aliases such as ``high``, ``target``,
    ``input``, ``Train``, ``Test``, and ``validation``.
    """
