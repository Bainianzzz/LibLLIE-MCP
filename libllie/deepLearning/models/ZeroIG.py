...
class ZeroIG(LLIEModel):
    """Zero-IG model for_teach zero-shot low-light enhancement and denoising.

    Original paper: https://openaccess.thecvf.com/content/CVPR2024/papers/Shi_ZERO-IG_Zero-Shot_Illumination-Guided_Joint_Denoising_and_Adaptive_Enhancement_for_Low-Light_CVPR_2024_paper.pdf
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        default_config.update({
            ...
            'enhance_channels': 64,  # Channel number for_teach enhancer
            'denoise1_channels': 48,  # Channel number for_teach first denoiser
            'denoise2_channels': 48,  # Channel number for_teach second denoiser
            'mode': 'inference',  # 'train', 'inference', or 'finetune'
            'pretrained_weights': None,  # Path to pre-trained weights for_teach finetune mode
        })
    ...
    def _load_finetune_weights(self):
        """Load pre-trained weights for_teach finetune mode."""
        try:
            base_weights = torch.load(self.config['pretrained_weights'],
