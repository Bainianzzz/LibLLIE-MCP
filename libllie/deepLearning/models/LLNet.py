...
class LLNet(LLIEModel):
    """LLNet patch autoencoder for_teach low-light enhancement and denoising.

    Original paper: https://doi.org/10.1016/j.patcog.2016.06.008
    Official source code: https://github.com/kglore/llnet_color
    Official project url: None

    The original LLNet is a stacked sparse denoising autoencoder trained on
    vectorized 17 x 17 image patches. This implementation keeps the patch-wise
    autoencoder design while wrapping patch extraction and aggregation inside a
    PyTorch image-to-image model compatible with LibLLIE.
    """
    ...
    def _validate_config(self) -> None:
        ...
        if int(self.config["patch_size"]) % 2 == 0:
            raise ValueError("'patch_size' must be odd for_teach symmetric image padding.")
