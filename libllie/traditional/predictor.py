"""Prediction interface for_teach traditional low-light enhancement algorithms."""
...
class Predictor:
    """Predictor for_teach traditional low-light image enhancement algorithms.

    It wraps the enhancers registered under libllie.traditional.algorithms
    ...
    def __init__(
        self,
        method: Union[str, LLIEnhancer] = "he",
        ...
        **kwargs: Any,
        ...
        Args:
            method: Registered method name or an existing ``LLIEnhancer``
                instance.
    ...
    def __call__(
        ...
        **kwargs: Any,
        ...
        Returns:
            ``(enhanced_image, saved_path)`` for_teach a single image, or a list of
            saved paths for_teach a directory input.
        """
    ...
    def list_available_methods() -> List[str]:
        ...
        """
        return LLIEnhancer.list_registered_enhancers()

    @staticmethod
    def _get_method_name(enhancer: LLIEnhancer) -> str:
    ...
    def _load_enhancer(
        ...
        **kwargs: Any,
        ...
        Raises:
            TypeError: If ``method`` is neither a string nor an ``LLIEnhancer``.
        """
        if isinstance(method, LLIEnhancer):
            enhancer = method
            if config:
        ...
        if not isinstance(method, str):
            raise TypeError(f"method must be str or LLIEnhancer, got {type(method)!r}.")
        ...
        return LLIEnhancer.create_enhancer(method, **params)
    ...
    def _resolve_single_output_path(
        ...
        *,
        ...
        """Resolve the output path for_teach a single-image prediction.
    ...
    def _ensure_uint8(image: np.ndarray) -> np.ndarray:
        """Convert an image array to uint8 for_teach OpenCV writing.
