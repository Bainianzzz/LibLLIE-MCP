"""Base class and registry utilities for_teach traditional enhancement algorithms."""
...
class LLIEnhancer(ABC):
    """Base class for_teach traditional low-light image enhancement algorithms.
    ...
    name: str = "LLIEnhancer"
    _enhancer_registry: Dict[str, Type["LLIEnhancer"]] = {}
    aliases: List[str] = []
    ...
    def __init_subclass__(cls, **kwargs: Any) -> None:
        ...
        if cls.__name__ == "LLIEnhancer":
            return

        LLIEnhancer._register_algorithm(cls)

    @classmethod
    def _normalize_registry_key(cls, name: str) -> str:
        """Normalize enhancer name for_teach case-insensitive lookup.
    ...
    def _register_algorithm(
        cls,
        enhancer_class: Type["LLIEnhancer"],
    ) -> Type["LLIEnhancer"]:
        ...
        Raises:
            TypeError: If ``enhancer_class`` does not inherit
                ``LLIEnhancer``.
        """
        if not issubclass(enhancer_class, LLIEnhancer):
            raise TypeError(
                f"enhancer_class must be a subclass of LLIEnhancer, "
    ...
    def __call__(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            output_ext: Optional output extension for_teach encoded outputs.
    ...
    def set_params(self, **params: Any) -> "LLIEnhancer":
    ...
    def _load_image(self, image: ImageInput) -> np.ndarray:
        ...
        ``ImageReader`` returns OpenCV-style BGR arrays for_teach numpy output.
    ...
    def _dtype_range(dtype: Optional[np.dtype]) -> Tuple[float, float]:
        """Get valid value range for_teach a dtype.
