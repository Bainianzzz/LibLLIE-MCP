...
In LibLLIE, the model is implemented as `KinDPlusPlus` because Python class names cannot contain `++`. The default YAML file keeps the paper-style name:

```text
libllie/deepLearning/config/KinD++.yaml
```

The LibLLIE implementation contains:
...
| Item | Location |
| --- | --- |
| Model implementation | `libllie/deepLearning/models/KinDPlusPlus.py` |
| Model class name | `KinDPlusPlus` |
| Default configuration | `libllie/deepLearning/config/KinD++.yaml` |
| Related loss | `libllie/deepLearning/loss/KinDPlusPlus_Loss.py` |
