...
CLAHE is a contrast-limited adaptive histogram equalization algorithm already implemented in LibLLIE.
...
| Item | Location |
| --- | --- |
| Algorithm implementation | `libllie/traditional/algorithms/CLAHE.py` |
...
LibLLIE's CLAHE is based on OpenCV's `cv2.createCLAHE()` and supports processing luminance channels or all channels in different color spaces.
