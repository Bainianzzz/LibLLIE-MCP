...
AHE is an adaptive histogram equalization algorithm already implemented in LibLLIE.
...
| Item | Location |
| --- | --- |
| Algorithm implementation | `libllie/traditional/algorithms/AHE.py` |
...
LibLLIE's AHE uses OpenCV's CLAHE interface and sets a large `clipLimit` to approximate AHE behavior.
